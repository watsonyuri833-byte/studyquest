...
# ==============================================================================
# BACKEND: GERENCIADOR DO SUPABASE (POSTGRESQL) E IA[cite: 3]
# ==============================================================================
class DatabaseManager:
  ...
  def init_db(self):
    # As tabelas já foram criadas via SQL Editor no Supabase,
    # mas mantemos o método caso precise de validações.
    pass
  ...
  def obter_config_geral(self, chave, default=""):
    with self.get_connection() as conn:
      with conn.cursor() as cursor:
        cursor.execute(
            "SELECT valor FROM config_geral WHERE chave = %s", (chave,)
        )
        res = cursor.fetchone()
  ...
  def remover_materia_edital(self, materia):
    with self.get_connection() as conn:
      with conn.cursor() as cursor:
        cursor.execute(
            "DELETE FROM edital_config WHERE materia = %s", (materia,)
        )
        conn.commit()
...
# Inicialização do Session State[cite: 2]
...
# ==============================================================================
# FRONTEND: STREAMLIT APP UI[cite: 2]
# ==============================================================================
  ...
  with st.form("form_cadastrar_questao"):
    ...
    if st.form_submit_button("💾 Salvar Questão Definitivamente"):
      if (
        ...
        st.success("Questão cadastrada com sucesso no Supabase!")
      else:
        st.warning("Preencha a Matéria, Enunciado e as Opções A e B!")

elif menu == "💾 Backup":
  st.title("💾 Gestão do Banco de Dados (Supabase)")
  st.info(
      "Seus dados estão armazenados de forma segura e permanente na nuvem do"
      " Supabase[cite: 3]. Para realizar backups completos ou gerenciar as"
      " tabelas diretamente, utilize o painel do Supabase na web (SQL Editor)."
  )